    ...
    <div class="management-hero">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center">
            <div>
                ...
                <p>{{ __('Highlight premium services and keep their visibility in sync.') }}</p>
            </div>
            <div class="mt-3 mt-md-0">
                ...
                <span class="stat-pill">
                    <i class="fas fa-home"></i> {{ $featuredItems }} {{ __('featured') }}
                </span>
                <span class="stat-pill">
    ...
    <div class="card management-card">
        <div class="card-header">
            <div class="d-flex flex-column flex-md-row justify-content-between gap-2">
                <div>
                    <h4 class="mb-1">{{ $modelName }} {{ __('Portfolio') }}</h4>
                    <p class="text-muted mb-0">{{ __('Control homepage presence and activation from a single table.') }}</p>
                </div>
                    ...
                    <tbody>
                        @forelse ($items as $item)
                            <tr>
                                ...
                                <td>
                                    @if ($item->image_path)
                                        <img src="{{ asset('storage/' . $item->image_path) }}" alt="{{ $item->translations->first()->name ?? __('Service image') }}" class="media-thumb">
                                    @else
                        ...
                        @empty
                            <tr class="empty-row">
                                <td colspan="6" class="text-center text-muted py-4">
                                    <i class="fas fa-info-circle mr-1"></i> {{ __('No services found yet.') }}
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        $(function () {
            ...
            $table.DataTable({
                ...
                language: {
                    ...
                    zeroRecords: "{{ __('No matching services found') }}",
                    paginate: {
                        first: "{{ __('First') }}",
