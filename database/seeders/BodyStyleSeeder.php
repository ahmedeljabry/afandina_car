...
class BodyStyleSeeder extends Seeder
{
    public function run()
    {
        ...
        DB::table('body_style_translations')->truncate();
        DB::table('body_styles')->truncate();

        // Re-enable foreign key checks after truncating
        DB::statement('SET FOREIGN_KEY_CHECKS=1;');
        // List of 15 popular car body_styles for car rentals in Dubai
        $body_styles = [
            [
            ...
            // Add more body_styles in a similar format to reach 15 body_styles
            [
            ...
            // Additional body_styles: Kia, Hyundai, Land Rover, and so on...
        ];

        foreach ($body_styles as $body_style) {
            $existingbody_style = DB::table('body_styles')->where('logo_path', $body_style['logo_path'])->first();

            if ($existingbody_style) {
                $body_styleId = $existingbody_style->id;
            } else {
                $body_styleId = DB::table('body_styles')->insertGetId([
                    'is_active' => $body_style['is_active'],
                    'logo_path' => $body_style['logo_path'],
                    'created_at' => Carbon::now(),
                    'updated_at' => Carbon::now(),
                ]);
            }

            foreach ($body_style['translations'] as $translation) {
                $metaKeywordsArray = explode(',', $translation['meta_keywords']);
                $metaKeywords = array_map(function ($keyword) {
                    return ['value' => trim($keyword)];
                }, $metaKeywordsArray);
                DB::table('body_style_translations')->insert([
                    'body_style_id' => $body_styleId,
